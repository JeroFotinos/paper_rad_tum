...
Cancer stem cells are often reported to survive radiotherapy better than non-stem cancer cells, a finding commonly attributed to intrinsic differences such as enhanced DNA repair or altered redox balance. However, tumors are spatially structured and contain strong oxygen gradients, which can also shape treatment response.

Here we use a three-dimensional computational tumorsphere model to test whether spatial organization alone can generate an apparent survival advantage. The model predicts that cancer stem cells become preferentially located toward the tumorsphere interior, where oxygen levels are lower. When we simulate a uniform radiation dose and assume identical intrinsic radiosensitivity for both phenotypes, interior cells are less likely to be killed because hypoxia reduces radiation effectiveness. As a result, cancer stem cells are enriched among survivors even without any built-in radioresistance. This enrichment vanishes when stemness labels are randomized and persists when a necrotic core is included.

These results show that spatial localization and microenvironmental oxygenation can be sufficient to explain cancer stem cell enrichment after radiotherapy, emphasizing the need to account for tumor architecture when interpreting treatment outcomes.
