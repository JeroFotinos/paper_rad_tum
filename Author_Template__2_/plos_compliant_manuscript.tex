% --- PLOS template header (keep all required packages from template) ---
...
% --- your extra packages (allowed to add) ---
...
% --- user macros ---
% (Keep your lstinline usage; PLOS allows it.)
% Add any additional macros you use:
% \newcommand{...}{...}

\begin{document}
	\vspace*{0.2in}
% --- Title/authors/affiliations in PLOS format ---
\begin{flushleft}
	{\Large
		\textbf{Spatial distribution of cancer stem cells promotes radioresistance in tumorspheres}
	}
	\newline
	
	Jer\'onimo Fotin\'os\textsuperscript{1,2*},
	Carlos A. Condat\textsuperscript{1,2},
	Carlos S. Bederi\'an\textsuperscript{2},
	Lucas Barberis\textsuperscript{1,2}
	\\
	\bigskip
	
	\textbf{1} Facultad de Matem\'atica, Astronom\'ia, F\'isica y Computaci\'on, Universidad Nacional de C\'ordoba, C\'ordoba, Argentina\\
	\textbf{2} Instituto de F\'isica Enrique Gaviola, CONICET, C\'ordoba, Argentina\\
	\bigskip
	
	* jerofotinos@gmail.com
\end{flushleft}
\section*{Abstract}
...
Radiotherapy is widely used to treat cancer, yet tumors often recover after treatment. Many studies link this therapeutical failure to cancer stem cells, a small subpopulation thought to drive regrowth. These cells are typically assumed to survive because they are intrinsically more radiation-resistant. But tumors are spatially organized, and a cell location can influence how much radiation damage it sustains.

We built a three-dimensional computer model of tumorspheres (spherical tumor-like cell clusters) that tracks each cell position during growth. Starting from a single stem cell, the model predicts that stem cells concentrate toward the spheroid interior. The interior is less oxygenated, and low oxygen levels reduce the effectiveness of radiotherapy. Therefore, when we apply a uniform radiation dose, inner cells are likelier to survive, leading to a relative enrichment of stem cells even when stem and non-stem cells are assigned the same intrinsic sensitivity to radiation.

To test that this effect is genuinely spatial, we repeated the treatment after randomly redistributing stem cells within the same spheroids and found a marked reduction in stem-cell survival, a conclusion that also holds when a necrotic core is present. This suggests that spatial organization should be considered when interpreting resistance to radiotherapy.
...
\subsection*{30-day Tumorspheres with a Necrotic Core}
...
\subsection*{The 60-day-old System}
