...
Radiotherapy is widely used to treat cancer, yet tumors often recover after treatment. Many studies link this failure to cancer stem cells, a small subpopulation thought to seed regrowth. These cells are typically assumed to survive because they are intrinsically more resistant. But tumors are spatially organized, and where a cell sits can change how much radiation damage it experiences.

We built a three-dimensional computer model of tumorspheres (spherical tumor-like cell clusters) that tracks each cell’s position during growth. Starting from a single stem cell, the model predicts that stem cells become concentrated toward the spheroid interior. The interior is less oxygenated, and low oxygen levels are known to reduce the effectiveness of radiotherapy. When we apply a uniform radiation dose in the model, inner cells are therefore more likely to survive, producing an apparent enrichment of stem cells even when stem and non-stem cells are assigned the same intrinsic sensitivity to radiation.

To test that this effect is truly spatial, we repeated the treatment after randomly redistributing stem cells within the same spheroids and found markedly lower stem-cell survival. The same conclusion holds when a necrotic core is present. Our results suggest that spatial organization should be considered when interpreting resistance to radiotherapy.
